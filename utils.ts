...
import { piloteData, playersData } from './data/piloteData'
import { raceResultData } from './data/resultData'
import { Pilote, PiloteName } from './types/pilote'
...
export const getPiloteLeagueResults = (withBots = true): PiloteResultWithTeam[] => {
  const data: Pilote[] = withBots ? piloteData : playersData
  const scores = data.map((pilote) => getSinglePiloteLeagueResult(pilote.name))
